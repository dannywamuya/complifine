...
import { and, createDatabase, eq, isNotNull, type Database } from "@complifine/db";
import {
  requirementVersions,
  standardDocuments,
  standards,
...
import { recordReview, transitionVersion, TransitionError } from "./steps/publish.ts";
  ...
  try {
    switch (args.command) {
      ...
      case "link":
        ...
        break;
      case "gates":
        return await commandGates(db, flagString(args, "version"), flagList(args, "gate"));
      case "all":
...
async function commandRegistry(db: Database): Promise<void> {
  ...
  await runJob({ db, runId, stage: "registry", echo: true }, (ctx) => syncRegistry(db, ctx));
}
...
async function commandFetch(
  ...
  heading("Fetch");
  const runId = newRunId();
  const summary = await runJob({ db, runId, stage: "fetch", echo: true }, (ctx) =>
    fetchDocuments(db, ctx, { slugs, force, localBaseDir: REPO_ROOT }),
  );
  ...
  if (summary.failed > 0) {
    ...
    for (const failure of summary.failures) {
      console.log(`  ${CROSS} ${failure.slug}: ${failure.error.split("\n")[0]}`);
    }
    throw new Error(
...
async function commandParse(db: Database, versionCode?: string): Promise<void> {
  ...
  for (const version of await selectVersions(db, versionCode)) {
    ...
    if (!checklist?.storageKey) {
      console.log(`  ${WARN} ${version.code}: no fetched checklist workbook, skipping`);
      continue;
    }
    ...
    await db
      ...
      .where(eq(standardDocuments.id, checklist.id));
  }
}
...
async function commandProse(db: Database, versionCode?: string): Promise<void> {
  ...
  for (const version of await selectVersions(db, versionCode)) {
    ...
    const prose = documents.filter(
      (d) => d.documentType === "general_regulations" || d.documentType === "guidance",
    );
...
async function commandLink(db: Database): Promise<void> {
  ...
  const runId = newRunId();
  const report = await runJob({ db, runId, stage: "reconcile", echo: true }, (ctx) =>
    linkEditions(db, ctx, {
...
async function commandGates(
  ...
  for (const version of await selectVersions(db, versionCode)) {
    ...
    for (const result of report.results) {
      ...
      const label = result.blocking ? result.gate : `${result.gate} ${style.dim("(advisory)")}`;
      console.log(`  ${icon} ${style.bold(label)}`);
      console.log(`      ${style.dim(result.description)}`);
      if (!result.passed || !result.blocking) {
        console.log(`      expected: ${result.expected}`);
        console.log(`      actual:   ${result.passed ? style.green(result.actual) : style.red(result.actual)}`);
      }
      if (!result.passed && result.failures?.length) {
        ...
        if (result.failures.length > preview.length) {
          console.log(`        ${style.dim(`... and ${result.failures.length - preview.length} more`)}`);
        }
      }
    }
    ...
    if (report.passed) {
      console.log(
        `  ${CHECK} ${style.green(`All ${report.results.filter((r) => r.blocking).length} blocking gates passed`)}` +
          (report.advisoryFailures ? style.dim(` (${report.advisoryFailures} advisory notes)`) : ""),
      );
    } else {
      console.log(`  ${CROSS} ${style.red(`${report.blockingFailures} blocking gate(s) failed`)}`);
      allPassed = false;
    }
  }
...
async function commandReview(
  ...
  if (!versionCode) throw new Error("Usage: kb review <version-code> --reviewer \"Name\" --decision approved");

  const reviewer = flagString(args, "reviewer");
  if (!reviewer) throw new Error("--reviewer is required: reviews must name a person.");
  ...
  console.log(`${CHECK} Recorded ${decision} for ${versionCode} by ${reviewer}`);
}
...
async function commandPromote(
  ...
  if (!versionCode) throw new Error("Usage: kb promote <version-code> --to <status>");
  ...
  const target = args.command === "publish" ? "published" : flagString(args, "to");
  if (!target) throw new Error("--to is required, e.g. --to review");

  const path =
    args.command === "publish"
      ? (["ingesting", "extracted", "validation", "review", "approved", "published"] as const)
      : ([target] as const);
...
async function commandStatus(db: Database, json: boolean): Promise<void> {
  ...
  for (const version of versions) {
    ...
    const requirements = await db
      .select({ level: requirementVersions.level, page: requirementVersions.sourcePage })
    ...
    rows.push({
      ...
      levels: Object.fromEntries(
        (["major_must", "minor_must", "recommendation"] as const).map((level) => [
          level,
  ...
  if (!standard) {
    console.log(`  ${WARN} Nothing registered yet. Run ${style.bold("bun run kb all")}.`);
    return;
  }

  console.log(`  ${style.bold(standard.name)}  ${style.dim(standard.publisher)}\n`);

  table([
    [style.dim("VERSION"), style.dim("STATUS"), style.dim("DOCS"), style.dim("SECTIONS"), style.dim("CRITERIA"), style.dim("PAGES"), style.dim("MAJOR/MINOR/REC")],
    ...rows.map((r) => {
  ...
  const manifestTotal = MANIFEST.flatMap((s) => s.versions).flatMap((v) => v.documents).length;
...
async function commandShow(
  ...
  for (const row of rows) {
    heading(`${row.requirement.sourceRequirementId}  ${style.dim(row.version.name)}`);
    table([
      ["  Level", style.bold(REQUIREMENT_LEVEL_LABELS[row.requirement.level])],
      ["  Section", row.section ? `${row.section.sourceIdentifier ?? ""} ${row.section.title}`.trim() : "-"],
      ["  Page", row.requirement.sourcePage ? `p.${row.requirement.sourcePage}` : "-"],
...
async function commandDiff(db: Database, json: boolean): Promise<void> {
  ...
  if (report.escalations.length) {
    console.log(`  ${style.bold(`Stricter in GFS (${report.escalations.length})`)}`);
    table(
      report.escalations.map((d) => [
        `    ${d.sourceRequirementId}`,
        style.dim(REQUIREMENT_LEVEL_LABELS[d.smartLevel]),
        style.dim("->"),
        style.yellow(REQUIREMENT_LEVEL_LABELS[d.gfsLevel]),
      ]),
    );
    console.log();
  }
  ...
  if (report.textChanges.length) {
    console.log(`  ${style.bold(`Reworded in GFS (${report.textChanges.length})`)}`);
    table(
...
async function commandWatch(db: Database): Promise<number> {
  ...
  if (report.changed.length === 0 && report.unreachable.length === 0 && report.undeclared.length === 0) {
    console.log(`  ${CHECK} ${style.green(`All ${report.checked} documents unchanged`)}`);
    return 0;
  }
  ...
  for (const item of report.unreachable) {
    console.log(`  ${CROSS} ${style.red("unreachable")}  ${item.slug}: ${item.reason}`);
  }
...
async function selectVersions(db: Database, versionCode?: string) {
  ...
  if (!versionCode) {
    if (versions.length === 0) {
      throw new Error("No standard versions registered. Run `bun run kb registry` first.");
    }
    return versions;
  }
