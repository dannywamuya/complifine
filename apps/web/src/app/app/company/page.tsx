...
} from '@/components/scoping-questions';
...
export default function CompanyPage() {
	...
	const [answers, setAnswers] = useState<
		Record<string, 'yes' | 'no' | 'unanswered'>
	>({});
	...
	const answered = useMemo(
		() =>
			questions.filter((q) => answers[q.id] === 'yes' || answers[q.id] === 'no')
				.length,
		[answers, questions],
	);
	...
	const scopedEditions = data?.scopes.length
		? data.scopes.map((scope) => ({ value: scope.code, label: scope.name }))
		: editions;
	const attachedCodes = new Set(data?.scopes.map((scope) => scope.code) ?? []);
	const addableEditions = editions.filter(
		(edition) => !attachedCodes.has(edition.value),
	);
	...
	async function attachStandard(code: string) {
		...
		setVersion(code);
		const applicability = await api<{ questions: ScopingQuestion[] }>(
			`/versions/${code}/applicability`,
		).catch(() => ({ questions: [] as ScopingQuestion[] }));
		if (applicability.questions.length === 0) {
			toast.success(
				'Standard attached. This edition has no scoping questions.',
			);
			return;
		}
		if (payload.sites.length === 0) {
			setTab('sites');
			toast.success(
				'Standard attached. Add a site, then answer what applies there.',
			);
			return;
		}
			...
			<div className='mx-auto flex w-full max-w-lg flex-col gap-8'>
				<header className='space-y-2'>
					...
					<p className='max-w-lg text-base leading-relaxed text-muted-foreground'>
						The company is the name on the certificate. Sites sit under it —
						growing, packing, or storage — and you will add those next.
					</p>
				</header>
		...
		<div className='mx-auto flex w-full max-w-4xl flex-col gap-8'>
			<header className='w-fit max-w-full space-y-2' id='tour-company'>
				...
				<p className='max-w-2xl text-base leading-relaxed text-muted-foreground'>
					...
					{data.organization.sedexZc
						? ` · Sedex ${data.organization.sedexZc}`
						: ''}
					{' · '}
			...
			<Tabs value={tab} onValueChange={setTab} className='gap-6'>
				<TabsList
					id='tour-company-tabs'
					className='h-10 w-fit max-w-full rounded-full bg-muted p-1'>
				...
				<TabsContent value='sites' className='mt-0 space-y-4'>
					<div
						id='tour-company-sites'
						className='flex w-fit max-w-full items-center justify-between gap-3'>
						...
						<AddSiteDialog
							...
							onCreated={async (site) => {
								...
								setSiteId(site.id);
								toast.success(
									'Site added. Answer what applies for the standards in scope.',
								);
					...
					{data.sites.length === 0 ? (
						<div className='rounded-2xl border border-dashed border-border bg-muted/60 px-6 py-12 text-center'>
							...
							<p className='mx-auto mt-1 max-w-sm text-sm leading-relaxed text-muted-foreground'>
								Pick a type: growing, packing, collection, or storage. Questions
								are answered per site, so two sites can differ.
							</p>
						</div>
					) : (
						<div className='grid gap-3 sm:grid-cols-2'>
							{data.sites.map((site) => {
								const Icon =
									site.siteType === 'packhouse' || site.siteType === 'warehouse'
										? Warehouse
										: Tractor;
				...
				<TabsContent value='scope' className='mt-0'>
					<div className='rounded-2xl border border-border bg-card p-6 shadow-[0_8px_28px_rgb(0_0_0/0.04)]'>
						<div className='mb-5 space-y-1'>
							...
							<p className='text-sm text-muted-foreground'>
								Editions this company is certified against or preparing for.
								Chat cites only these.
							</p>
						</div>
						<div className='space-y-4'>
							...
							{addableEditions.length === 0 ? (
								<p className='text-sm text-muted-foreground'>
									All published editions are already in scope.
								</p>
							) : (
								<form
									...
									<div className='flex items-center gap-1.5 mt-2'>
										...
										<InfoHint label='Why add a standard?'>
											These are the editions Chat is allowed to cite. Attach
											every one you are certified against or preparing for.
										</InfoHint>
									</div>
								...
								<div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
									<div className='space-y-1'>
										<h2 className='flex items-center gap-1.5 font-heading text-base font-medium tracking-tight'>
											...
											<InfoHint label='Why are we asking these questions?'>
												{SCOPING_WHY}
											</InfoHint>
										</h2>
										<p className='text-sm text-muted-foreground'>
											...
											{selected
												? ` (${SITE_TYPE_LABELS[selected.siteType] ?? selected.siteType})`
												: ''}
									...
									try {
										...
										setResult(
											await api<Resolution>(
												`/sites/${siteId}/resolution?versionCode=${version}`,
											),
										);
				...
				<TabsContent value='organisation' className='mt-0'>
					<div className='rounded-2xl border border-border bg-card p-6 shadow-[0_8px_28px_rgb(0_0_0/0.04)]'>
						<div className='mb-5 space-y-1'>
							...
							<p className='text-sm text-muted-foreground'>
								The name on the certificate. Sites live under Sites. Sedex ZC is
								a membership number, not a second standard.
							</p>
						</div>
						<form
							...
							<div className='space-y-1.5'>
								<div className='flex items-center gap-1.5'>
									...
									<InfoHint label='What is a Sedex ZC?'>
										Your Sedex membership number if you have one. It is a
										platform ID, not a second standard.
									</InfoHint>
								</div>
								<Input
									id='org-sedex'
			...
			<DialogContent>
				<form
					...
					<DialogHeader>
						...
						<DialogDescription>
							A site is a place this company operates. Certificates sit on the
							company; the work happens here.
						</DialogDescription>
					</DialogHeader>
					<div className='space-y-3'>
						<div className='space-y-1.5'>
							...
							<Input
								...
								name='name'
								required
								placeholder='Naivasha packhouse'
							/>
						</div>
						<div className='space-y-1.5'>
							<div className='flex items-center gap-1.5'>
								...
								<InfoHint label='Why does site type matter?'>
									Chat treats a field, packhouse, collection centre, and
									warehouse as different places. Pick the type that matches the
									work at your site.
								</InfoHint>
							</div>
